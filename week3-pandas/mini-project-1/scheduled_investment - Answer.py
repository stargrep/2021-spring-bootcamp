...
def export_result() -> float:
    ...
    df_new = pd.DataFrame(calculate_scheduled_investment(RAW_DATA_NAME))
    RAW_DATA_NAME.join(df_new)
    write_data(RAW_DATA_NAME)
    asset = [df_new.asset]  # replace
    cost = [df_new.cost]  # replace
    print(df_new)
